...
> C++11 后，C++98 的 **Rule of three** (Destructor / Copy Constructor / Copy Assignment Operator) --> C++11 的 **Rule of five** (plus Move Constructor / Move Assignment Operator) / **Rule of zero**
...
- **缺点**：当你传入 **字符串字面量**（"hello"）或 **子串** 时，它会发生昂贵的 **内存分配**
    ...
    - 把"hello world"拷贝进去
    - 函数结束后再释放临时对象的内存
    - 比如 `std::string` 的 API `substr()`：`long_str.substr(0, 5)` 会做深拷贝返回一个新的 `std::string` 对象
...
- `std::chrono::steady_clock`：秒表，单调递增，在计算 benchmark 时需要用这个
